...
function App() {
  ...
  // useEffect(() => {
  //   console.log("counter is set in use effect")
  ...
  // this (the one below) will run one time for sure and will run if the state variable is updated or the dom is updated
  ...
  // runs (below) one time and then one more time when the dom updates and thats it 
...
// ]
  ...
  return (
    <div className="App">
      ...
      />
      {pokemon?.filter((pokemon) =>pokemon.name.includes(filter)).map((pokemon) => {
        return (
          <Pokemon
            key={pokemon.name}
            pokemon={pokemon}
